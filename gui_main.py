...
#
    ...
    def setPlotControlLayout(self):
        ...
        self.plotControlLayout.addWidget(self.persistentFramesInput,3,0,1,1)
        self.plotControlLayout.addWidget(self.pFILabel,3,1,1,1)
        self.plotControlLayout.addWidget(self.staticclutter,4,0,1,1)
    ...
    def updateGraph(self, parsedData):
        ...
        if (numPoints):
            self.previousCloud[:5,:numPoints,fNum] = pointCloud
        ...
        if (self.graphFin):
            ...
            if (self.threeD):
                try:
                    ...
                    indicesIn = []
                self.get_thread = updateQTTargetThread3D(pointIn, targets, indicesIn, self.scatter, self.pcplot, numTargets, self.ellipsoids, classifierOutput, self.zRange, self.gw, self.plotByIndex.isChecked(), self.plotTracks.isChecked(), self.bbox,self.boundaryBoxes[0]['checkEnable'].isChecked())
