...
class updateQTTargetThread3D(QThread):
    ...
    def __init__(self, pointCloud, targets, indexes, scatter, pcplot, numTargets, ellipsoids, classifierOut=[], zRange=6, gw=[], colorByIndex=False, drawTracks=True, bbox=[0,0,0,0,0,0], bbox_en=0):
        QThread.__init__(self)
        self.pointCloud = pointCloud
        self.targets = targets
        self.indexes = indexes
    ...
    def run(self):
        ...
        if (self.colorByIndex):
            ...
            for i in range(np.shape(self.pointCloud)[1]):
                ...
                if (zs < 0) or (zs > self.zRange):
                    colors[i]=pg.glColor('k')
                else:
                    colors[i]=pg.glColor(self.gw.getColor(zs/self.zRange))
            self.scatter.setData(pos=toPlot, color=colors, size=size)
        #graph the targets
        if (self.drawTracks):
